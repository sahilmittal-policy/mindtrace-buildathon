...
(function (root, factory) {
  ...
  function scoreSymbolMatch(metrics) {
    ...
    const correct = typeof metrics.correct === "number" ? metrics.correct : Math.max(0, rounds - (metrics.skipped || 0));
    const accuracy = clamp(correct / rounds, 0, 1);
    const pace = typeof metrics.avgSeconds === "number" ? metrics.avgSeconds : 12;
    const speed = clamp(100 - (pace - 3) * 11, 0, 100);
    return {
      speed: clamp(speed * 0.62 + accuracy * 100 * 0.38, 0, 100),
      focus: clamp(accuracy * 100 * 0.7 + speed * 0.3 - (metrics.wrongTaps || 0) * 3 - (metrics.skipped || 0) * 5, 0, 100)
    };
  }

  function scoreMemory(metrics) {
    const average = typeof metrics.avgScore === "number" ? metrics.avgScore : 0;
    return { memory: clamp(average * 100, 0, 100) };
  }

  function scoreWords(metrics) {
    const rounds = Math.max(1, metrics.rounds || 0);
    const accuracy = clamp((metrics.correct || 0) / rounds, 0, 1);
    const pace = typeof metrics.avgSeconds === "number" ? metrics.avgSeconds : 10;
    const timeScore = clamp(100 - Math.max(0, pace - 7) * 5, 0, 100);
    return { words: clamp(accuracy * 80 + timeScore * 0.2 - (metrics.skipped || 0) * 3, 0, 100) };
  }

  function calculateDomains(games) {
    const domains = {};
    if (games.symbolMatch) Object.assign(domains, scoreSymbolMatch(games.symbolMatch));
    if (games.memory) Object.assign(domains, scoreMemory(games.memory));
    if (games.words) Object.assign(domains, scoreWords(games.words));
    Object.keys(domains).forEach((key) => { domains[key] = round(domains[key]); });
    return domains;
  }

  function composite(domains) {
    const values = Object.values(domains).filter((value) => typeof value === "number");
    return values.length ? round(mean(values)) : null;
  }
  ...
  function getBaseline(sessions) {
    ...
    ["speed", "focus", "memory", "words"].forEach((key) => {
      const values = window.map((session) => session.domains && session.domains[key]).filter((value) => typeof value === "number");
      if (values.length) {
        const sd = standardDeviation(values);
        domains[key] = { mean: mean(values), sd, halfWidth: Math.max(3.5, sd) };
      }
    });
    const composites = window.map((session) => session.composite).filter((value) => typeof value === "number");
    if (composites.length) {
      const sd = standardDeviation(composites);
      domains.composite = { mean: mean(composites), sd, halfWidth: Math.max(3.5, sd) };
    }
    return { fromSession: 8, toSession: 14, domains };
  }

  function verdictFor(sessions, context) {
    const count = sessions.length;
    if (count < 15) {
      return { key: "building", label: "Building your picture", explanation: `Complete ${15 - count} more ${15 - count === 1 ? "session" : "sessions"} to begin your personal usual range.` };
    }
    const baseline = getBaseline(sessions);
    const latest = sessions[sessions.length - 1];
    const band = baseline.domains.composite;
    if (!band || !latest) {
      return { key: "building", label: "Building your picture", explanation: "Your personal range is still taking shape." };
    }
    const recent = sessions.slice(-3);
    const belowCount = recent.filter((session) => typeof session.composite === "number" && session.composite < band.mean - band.halfWidth).length;
    const lowDomains = ["speed", "focus", "memory", "words"].filter((key) => {
      ...
      return entry && typeof value === "number" && value < entry.mean - entry.halfWidth * 1.5;
    });
    const checkin = latest.checkin || {};
    const poorSleep = (checkin.sleepHours ?? 8) < 5.5 || (checkin.sleepQuality ?? 3) <= 2;
    const highStress = (checkin.stress ?? 1) >= 4;
    const contextText = poorSleep || highStress
      ? "Sleep or stress may be part of this."
      : "Sleep and stress have been in their usual range, so those do not appear to explain it.";

    if (belowCount >= 2) {
      return { key: "flag", label: "Worth discussing", explanation: `${formatAreas(lowDomains)} are below your usual range across recent check-ins. ${contextText} This is not a medical diagnosis. Mention it at your next appointment.` };
    }
    if (typeof latest.composite === "number" && latest.composite < band.mean - band.halfWidth) {
      return { key: "context", label: "A day with context", explanation: `Today's result is below your usual range, and sleep or stress may be influencing it. See how you feel after a more settled day.` };
    }
    if (lowDomains.length) {
      return { key: "note", label: "Worth watching", explanation: `${formatAreas(lowDomains)} are a little below your usual range today. One day is only one day; keep checking in.` };
    }
    return { key: "good", label: "Within your usual range", explanation: "Today's result sits within the range you've built for yourself." };
  }
  ...
  function longestStreak(sessions) {
    ...
    days.forEach((date) => {
      ...
      if (prior && (current - prior) / 86400000 === 1) run += 1;
      else run = 1;
  ...
  /* ------------------------------------------------------------------ badges
   *
   * Every badge here rewards TURNING UP. None of them can be earned by
   * scoring well, and that is deliberate: a badge for a high score makes
   * people push harder on badge days, and that extra effort lands straight
   * in the signal MindTrace is trying to read.
   *
   * Each badge reports its own progress so the app can show how close the
   * next one is, which is the part that actually motivates.
   */
  ...
  function hadGap(sessions, gapDays) {
    ...
    for (let i = 1; i < dates.length; i += 1) {
      ...
      if ((b - a) / 86400000 > gapDays) return true;
    }
    return false;
  }
  ...
  function badgesFor(sessions) {
    ...
    const fullSets = sessions.filter((s) => Object.keys(s.games || {}).length === 3).length;
    const gamesTried = new Set();
    sessions.forEach((s) => Object.keys(s.games || {}).forEach((key) => gamesTried.add(key)));
    const shortNight = sessions.some((s) => s.checkin && s.checkin.sleepHours < 5.5);
    const heavyDay = sessions.some((s) => s.checkin && s.checkin.stress >= 5);
    const lowMood = sessions.some((s) => s.checkin && s.checkin.mood <= 2);
    const notedSomething = sessions.some((s) => s.checkin && s.checkin.notes && s.checkin.notes.length > 0);

    // tier drives the medal colour: 1 bronze, 2 silver, 3 gold
    const make = (key, name, detail, done, target, tier, cheer) => ({
      key, name, detail, tier,
      unlocked: done >= target,
      progress: Math.max(0, Math.min(1, target ? done / target : 0)),
      progressLabel: done >= target ? "Earned" : `${Math.floor(done)} of ${target}`,
      remaining: Math.max(0, target - done),
      cheer
    });

    return [
      make("first", "First Step", "Your very first check-in", count, 1, 1,
        "Everything starts here."),
      make("set", "Full Set", "All three games in one day", fullSets, 1, 1,
        "Try all three in one sitting — it makes a fuller picture."),
      make("explorer", "Curious Sort", "Tried every game at least once", gamesTried.size, 3, 1,
        "One more game to try."),
      make("three", "Three in a Row", "Three days running", best, 3, 1,
        "Three days is where a habit starts to hold."),
      make("noted", "Worth Remembering", "Added a note to a check-in", notedSomething ? 1 : 0, 1, 1,
        "Jot down anything unusual — it explains a lot later."),
      make("rain", "Rain or Shine", "Checked in after a short night", shortNight ? 1 : 0, 1, 2,
        "The honest days matter most."),
      make("storm", "Weathered It", "Checked in on a heavy day", heavyDay ? 1 : 0, 1, 2,
        "Showing up when it is hard is the whole point."),
      make("kind", "Kind to Yourself", "Checked in when your mood was low", lowMood ? 1 : 0, 1, 2,
        "No pressure. Just noticing counts."),
      make("week", "Full Week", "Seven days running", best, 7, 2,
        "A full week is a real achievement."),
      make("comeback", "Back Again", "Returned after a break of a week or more", hadGap(sessions, 7) ? 1 : 0, 1, 2,
        "Missed days are normal. Coming back is what counts."),
      make("baseline", "Baseline Built", "Fourteen check-ins — your range begins", count, 14, 3,
        "At fourteen, MindTrace starts comparing you with you."),
      make("thirty", "Thirty Sessions", "Thirty check-ins recorded", count, 30, 3,
        "Thirty days of your own history."),
      make("fortnight", "Fourteen Straight", "Fourteen days running", best, 14, 3,
        "Two unbroken weeks."),
      make("month", "Month Streak", "Thirty days running", best, 30, 3,
        "A month without a gap."),
      make("century", "Hundred Days", "One hundred check-ins", count, 100, 3,
        "The long view.")
    ];
  }

  /* The next badge to aim for, plus a line of encouragement about it. */
  function nextBadge(sessions) {
    const locked = badgesFor(sessions).filter((badge) => !badge.unlocked);
    if (!locked.length) return null;
    // Closest to completion first, so the suggestion always feels reachable.
    locked.sort((a, b) => b.progress - a.progress || a.remaining - b.remaining);
    return locked[0];
  }

  /* A short, honest nudge for the dashboard. Never mentions a score. */
  function encouragement(sessions) {
    ...
    if (away > 2) return `It has been ${away} days. Nothing is lost — pick up where you left off.`;
    if (count === 1) return "One down. The picture builds a day at a time.";
    if (count < 14) return `${14 - count} more check-ins until MindTrace can compare a day with your own usual range.`;
    if (count === 14) return "Fourteen in. From here, each day is measured against your own rhythm.";
    if (now >= 7) return `${now} days running. That is a real habit now.`;
    if (now >= 3) return `${now} days in a row. Nicely steady.`;
